...
CAPABILITIES: tuple[Capability, ...] = (
    ...
    Capability(
        ...
        kind="tool",
        source_hint="mcp_server.py / skills/content_model/skill.py",
        responsibility="Score email body and sender text with a calibrated low-false-positive text classifier.",
        trigger_terms=("content classifier", "text classifier", "body text", "content model", "正文分类", "文本分类"),
    ),
    Capability(
    ...
    Capability(
        ...
        kind="tool",
        source_hint="mcp_server.py / skills/scam_indicators/skill.py",
        responsibility="Detect obvious phishing and scam indicators in sender/content wording.",
        trigger_terms=("scam", "gift card", "crypto", "impersonation", "lookalike", "诈骗", "仿冒"),
    ),
    Capability(
        name="spam_campaign_check",
        kind="tool",
        source_hint="mcp_server.py / skills/spam_campaign/skill.py",
        responsibility="Detect high-precision bulk spam campaign patterns in message text.",
        trigger_terms=("spam campaign", "bulk spam", "stock spam", "pharmacy spam", "垃圾营销", "垃圾广告"),
    ),
    Capability(
...
def tool_usage_guidance() -> list[str]:
    return [
        ...
        "- Use `content_model_check` when the email mostly consists of body text, or when headers are incomplete and you still need a calibrated low-FPR verdict.",
        ...
        "- Use `scam_indicator_check` when impersonation, account takeover, gift-card, crypto, or extortion cues are present.",
        "- Use `spam_campaign_check` when you suspect bulk spam, advertising spam, stock spam, pharmacy spam, or template-driven campaigns.",
