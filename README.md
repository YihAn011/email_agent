...
`email_agent` is a terminal chatbot for email security analysis. It uses a local Ollama-hosted `Qwen3` model by default, exposes local capabilities through an MCP server, scans raw emails with `rspamd`, checks headers for SPF/DKIM/DMARC signals, and can bind a mailbox over IMAP to monitor new emails or scan the latest emails on demand.
...
If you want to keep Gemini as a fallback, you can also set:
